//This is a very large array of names to randomize
...
//These arrays are not as long, but for same purpose
const arrayOfSkills = ["Dodgeball", "Animal Tamer", "Astronaut", "Navigator", "Politician", "Barista", "Fisher", "Gymnist", "Fearless", "Rapper", "Circus Preformer", "Breath Holding", "Talking", "Watching Paint Dry", "Gaming", "Sewing", "Sketching", "Fighting", "Skiing", "Binge-watching Netflix", "Speed reading", "Building", "Driving", "Linguist", "Cooking", "Cleaning", "Engineer", "Web-Developer", "Drinking", "Speed Eating", "Psychic", "Theif", "Sports-Enthusiest", "Liar", "Eating a big mac in one bite", "Acting", "Dancing", "Swimming", "Running", "Screaming", "Partying", "Gym-Rat"]
...
//these are all the randomizers
let randomName = () => {
    return Math.floor(Math.random() * 2737)}
let randomAge = () => {
    return Math.floor(Math.random() * 59) + 12}
let randomSkill = () => {
    return  Math.floor(Math.random() * 42)
}
let randomCity = () => {
    return Math.floor(Math.random() * 384)}
let coinFlip = () => {
...
//These will be the primary arrays players will be going in and out of as they move to each section
let listOfPlayers = [];
let candidates = [];
let dodgeBallPlayerArray = [];
const blueTeam = [];
const redTeam = [];
let idIndex = 0;
let listIndex = 0;
let currentTeamSelector = "blue";



class player {
  constructor(inputId, inputName, inputAge, inputSkillSet, inputPlaceBorn){
      this.id = inputId;
      this.name = inputName;
      this.age = inputAge; 
      this.skillSet = inputSkillSet;
...
class dodgeBallPlayer extends player{
    constructor(inputId, inputName, inputAge, inputSkillSet, inputPlaceBorn,
    inputCanThrowBall, inputCanDodgeBall, inputHasPaid, inputIsHealthy, inputYearsExperience){

        super(inputId, inputName, inputAge, inputSkillSet, inputPlaceBorn);
...
//functions are in constructors to randomize the attributes of each player
let createNewPlayers = () => {
    for (let i = 0; i < 10; i++) {
        //This is Name
        let placeHolderFirst = names[randomName()]
        let placeHolderSecond = names[randomName()]
        let placeHolderFull = placeHolderFirst + " " + placeHolderSecond
        //Age is in constructor
        //Skill is in constructor
        //PlaceBorn is in constructor
        new player (idIndex, placeHolderFull,randomAge(), arrayOfSkills[randomSkill()], names[randomCity()])
        //This helps prevent bugs when adding new players
        idIndex++;
    }
  
    const listPlayersInDom = () => {
      //this puts players into the dom
      const listElement = document.getElementById('people')
      listOfPlayers.forEach(function(person, index){
        const li = document.createElement(`li`)
        li.id = `LOPli${listIndex}`
        ...
        candidates.push(listOfPlayers[index])
        //this is a placeholder value to make sure every new player gets a new ID
        listIndex++
      })
    }
 
    listPlayersInDom()
    //list of players array is emptied so forEach method can function properly each time we add new players
    listOfPlayers=[]
  ...
  const makePlayer = (ID) => {
    //coinflips are placed into the argument for the constructor to randomize true/false values
    //This is a random years of experience
    let randomYE = Math.floor(Math.random() * candidates[ID].age)

    dodgeBallPlayerArray.push(new dodgeBallPlayer (candidates[ID].id,candidates[ID].name,
      candidates[ID].age,candidates[ID].skillSet,candidates[ID].placeBorn,
      coinFlip(), coinFlip(), coinFlip(), coinFlip(), randomYE))
 
      
    //creates button in dom to place element on teams
    const dodgeBallPlayerElement = document.getElementById("players")
    let button = document.createElement(`button`)
    button.id = `dbp${ID}`
    button.setAttribute("onclick",`setTeam(this)`)
    button.appendChild(document.createTextNode(candidates[ID].name))
    dodgeBallPlayerElement.append(button)
    //removes selected player from DOM and array
    let removeLiFromDom = document.getElementById(`LOPli${ID}`)
    removeLiFromDom.remove()
    //had to splice from listOfPlayers, because after class is extended, it is created in the LOP array again
    //indicative of buggy code :( 
    listOfPlayers.splice(0,1)

    console.log(dodgeBallPlayerArray)
  }




  const setTeam = (element) => {
    //This grabs the id number of element for next part
    let elementID = element.id.slice(3,6)
    //uses that number to find it's position in the new array
    dodgeBallPlayerArray.forEach(function(element,index){
      if (element.id == elementID){
        //checks to see who's turn it is to gain a new member
        if (currentTeamSelector === "red"){
          redTeam.push(new redTeammate(dodgeBallPlayerArray[index].id,dodgeBallPlayerArray[index].name,dodgeBallPlayerArray[index].age,dodgeBallPlayerArray[index].skillSet,dodgeBallPlayerArray[index].placeBorn,
            dodgeBallPlayerArray[index].canThrowBall,dodgeBallPlayerArray[index].canDodgeBall,dodgeBallPlayerArray[index].hasPaid,dodgeBallPlayerArray[index].isHealthy,dodgeBallPlayerArray[index].yearsExperience,))
            currentTeamSelector = "blue";
            //grabs element and creates in DOM
            const htmlElementSelector = document.getElementById("red");
            const li = document.createElement('li')
            li.appendChild(document.createTextNode(dodgeBallPlayerArray[index].name))
            htmlElementSelector.append(li)
            //removes from DBP player list in DOM
            let removeButtonFromDom = document.getElementById(`dbp${elementID}`)
            removeButtonFromDom.remove()
            document.getElementById("nowSelecting").innerText = "Now selecting for team blue"

            console.log(redTeam)
        } else {
          //same as above but blue
          blueTeam.push(new blueTeammate(dodgeBallPlayerArray[index].id,dodgeBallPlayerArray[index].name,dodgeBallPlayerArray[index].age,dodgeBallPlayerArray[index].skillSet,dodgeBallPlayerArray[index].placeBorn,
            dodgeBallPlayerArray[index].canThrowBall,dodgeBallPlayerArray[index].canDodgeBall,dodgeBallPlayerArray[index].hasPaid,dodgeBallPlayerArray[index].isHealthy,dodgeBallPlayerArray[index].yearsExperience,))
            currentTeamSelector = "red";

            const htmlElementSelector = document.getElementById("blue");
            const li = document.createElement('li')
            li.appendChild(document.createTextNode(dodgeBallPlayerArray[index].name))
            htmlElementSelector.append(li)
            ...
            let removeButtonFromDom = document.getElementById(`dbp${elementID}`)
            removeButtonFromDom.remove()
            document.getElementById("nowSelecting").innerText = "Now selecting for team red"
...
//it "should create a new player using constructor"
//createNewPlayers()
//assert.deepEqual(candidates, [player {id:inputId, name:inputName, age:inputAge, skillSet:inputSkill, placeBorn:inputPlaceBorn}]x10)
//it "should list players in the dom"
//createNewPlayers()
//assert.deepEqual(testLi = document.getElementById('LOPli0').innerText, "MakePlayerFirstName LastName - Skill")
//describe setTeam(element)
//it "should set which team player is on, and alternate between red and blue"
//createNewPlayers()
//makePlayers(ID)
//setTeam(element) 
//assert.deepEqual(blueTeammate.length, 1)
//assert.deepEqual(currentTeamSelector, "red")
